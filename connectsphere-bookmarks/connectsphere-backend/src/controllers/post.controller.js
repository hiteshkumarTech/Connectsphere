...
// Annotates lean post objects with the viewer's own reaction + saved state.
async function attachMyReactions(posts, userId) {
  ...
  const reactionMap = new Map(reactions.map((r) => [String(r.target), r.type]));
  const savedSet = new Set(bookmarks.map((b) => String(b.post)));
  posts.forEach((p) => {
    p.myReaction = reactionMap.get(String(p._id)) || null;
...
const getPost = asyncHandler(async (req, res) => {
  ...
  if (req.user) {
    const [r, b] = await Promise.all([
      Reaction.findOne({ user: req.user.id, targetType: 'post', target: post._id })
        .select('type')
        .lean(),

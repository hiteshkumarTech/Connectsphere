...
import { MessageCircle, Share2, Bookmark, MoreHorizontal, Trash2, Pin, BadgeCheck } from 'lucide-react';
...
export default function PostCard({ post: initial, onDeleted, onUnsave }) {
  ...
  const toggleSave = async () => {
    ...
    setPost((p) => ({ ...p, saved: next })); // optimistic
    try {
      if (next) await api.post(`/bookmarks/${post._id}`);
      else await api.delete(`/bookmarks/${post._id}`);
      if (!next) onUnsave?.(post._id); // lets the Saved page drop it from the list
    } catch (err) {
      setPost((p) => ({ ...p, saved: !next })); // rollback
      toast(apiError(err), 'error');
    }
  };
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex items-center gap-1 text-slate-500">
            ...
            <button
              ...
              className={`ml-auto flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-sm font-medium transition hover:bg-slate-100 dark:hover:bg-slate-800 ${
                post.saved
